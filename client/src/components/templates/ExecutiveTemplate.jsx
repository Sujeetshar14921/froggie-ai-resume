...
const ExecutiveTemplate = ({ data, accentColor = "#7C3AED", pageContent = null }) => {
  ...
  const SectionHeading = ({ children, continuation = false }) => (
    <h2
      className="text-xs sm:text-sm font-bold uppercase tracking-[0.18em] mb-3 pb-1 border-b-2 flex items-center justify-between font-serif"
      style={{ color: accentColor, borderColor: accentColor }}
    >
      <span>{children}</span>
      {continuation && (
        <span className="text-[10px] lowercase font-normal opacity-70 font-sans">(continued)</span>
      )}
    </h2>
  );
  ...
  return (
    <div className="w-full bg-white text-slate-800 font-sans text-xs sm:text-sm leading-relaxed">
      {/* EXECUTIVE HEADER */}
      {showHeader && (
        <header data-resume-section="header" className="mb-5 pb-3 border-b-2" style={{ borderColor: `${accentColor}40` }}>
          <div className="flex flex-wrap justify-between items-end gap-2">
            <div>
              <h1 className="text-2xl sm:text-3xl font-bold tracking-wide uppercase text-slate-950 font-serif">
                {data?.personal_info?.full_name || "Your Name"}
              </h1>
              {data?.personal_info?.profession && (
                <p
                  className="mt-0.5 text-xs sm:text-sm uppercase tracking-[0.2em] font-semibold"
                  ...
                  return (
                    <React.Fragment key={index}>
                      ...
                      <Tag
                        ...
                        className={`inline-flex items-center gap-1 ${href ? "hover:underline text-slate-700 hover:text-slate-950" : ""}`}
                      >
      ...
      {showSummary && data?.professional_summary && (
        <section data-resume-section="summary" className="mb-5 break-inside-avoid">
          ...
          <div
            className="p-3 rounded-lg border-l-3 bg-slate-50/70"
        ...
        <section className="mb-5">
          {showExpTitle && (
            <div data-resume-section="experience-title">
              <SectionHeading continuation={isExpContinuation}>Leadership Experience</SectionHeading>
            </div>
          )}
        ...
        <section className="mb-5">
          {showProjTitle && (
            <div data-resume-section="projects-title">
              <SectionHeading continuation={isProjContinuation}>Strategic Projects & Initiatives</SectionHeading>
            </div>
          )}
        ...
        <section className="mb-5">
          {showEduTitle && (
            <div data-resume-section="education-title">
              <SectionHeading continuation={isEduContinuation}>Education & Credentials</SectionHeading>
            </div>
          )}
      ...
      {data?.certifications && data.certifications.length > 0 && (
        <section className="mb-5">
          <div data-resume-section="certifications-title">
            <SectionHeading>Certifications & Executive Programs</SectionHeading>
          </div>
      ...
      {data?.achievements && data.achievements.length > 0 && (
        <section className="mb-5">
          <div data-resume-section="achievements-title">
            <SectionHeading>Executive Honors & Distinctions</SectionHeading>
          </div>
          <div className="space-y-2.5">
        ...
        <section data-resume-section="skills" className="break-inside-avoid">
          ...
          <div className="flex flex-wrap gap-2">
            {data.skills.map((skill, index) => (
              <span
                key={index}
                className="text-xs font-semibold px-2.5 py-1 rounded bg-slate-100 text-slate-900 border border-slate-200"
              >
                {skill}
              </span>
            ))}
