...
const ClassicTemplate = ({ data, accentColor = "#1E3A8A", pageContent = null }) => {
  const SectionTitle = ({ children, continuation = false }) => (
    <h2
      className="text-xs sm:text-sm font-bold uppercase tracking-[0.16em] mb-3 pb-1 border-b-2 flex items-center justify-between font-serif"
  ...
  return (
    <div className="w-full bg-white text-slate-800 leading-relaxed font-sans text-xs sm:text-sm">
      {/* HEADER */}
      {showHeader && (
        <header data-resume-section="header" className="text-center mb-6 pb-3">
          <h1 className="text-2xl sm:text-3xl font-bold tracking-tight text-slate-900 uppercase font-serif">
            {data?.personal_info?.full_name || "Your Name"}
          </h1>

          {data?.personal_info?.profession && (
            <p
              className="mt-1 text-xs sm:text-sm font-semibold uppercase tracking-[0.2em]"
          ...
          {contactItems.length > 0 && (
            <div className="mt-3 flex flex-wrap justify-center items-center gap-x-3.5 gap-y-1 text-xs text-slate-600">
          ...
          <div
            className="mt-4 mx-auto w-24 h-0.5 rounded-full"
              ...
              return (
                ...
                >
                  <div className="flex flex-wrap justify-between items-baseline gap-x-4">
                    ...
                    <span className="text-[11px] font-medium text-slate-500 uppercase tracking-wider whitespace-nowrap">
      ...
      {data?.achievements && data.achievements.length > 0 && (
        <section className="mb-5">
          <div data-resume-section="achievements-title">
            <SectionTitle>Honors & Key Achievements</SectionTitle>
          </div>
          <div className="space-y-2.5">
        ...
        <section data-resume-section="skills" className="break-inside-avoid">
          ...
          <div className="flex gap-1.5 flex-wrap">
            {data.skills.map((skill, index) => (
              <span
                key={index}
                className="text-xs font-semibold px-2.5 py-0.5 rounded border text-slate-800"
                style={{
